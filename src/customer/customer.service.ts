...
export class CustomerService {
  ...
  async singout(id: number, logoutUserDto: LogoutUserDto){
    ...
    const user = await this.customerRepo.findOne({where: {id: id}})
    ...
    const updatedUser = await this.customerRepo.update({hashed_refresh_token: null}, {
  ...
  update(id: number, updateCustomerDto: UpdateCustomerDto) {
    return this.customerRepo.update(updateCustomerDto, {where: {id: id}, returning: true});
  }

  remove(id: number) {
    return this.customerRepo.destroy({where: {id: id}});
  }
}
